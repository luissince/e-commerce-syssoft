import { fetchListFlavor } from "@/app/lib/data";
...
export default async function ListFlavors() {
    const flavors = await fetchListFlavor();
    return (
        <ul>
            {flavors.map((item: AttributeModel, index: number) => (
                <Item key={index} {...item} />
            ))}
        </ul>
    );
